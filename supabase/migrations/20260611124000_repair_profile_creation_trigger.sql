-- PinayMate profile creation trigger repair.
--
-- Purpose:
-- - Recreate the auth.users -> public.profiles lifecycle trigger as an
--   idempotent, launch-tracked migration.
-- - Preserve existing onboarding values if a client fallback profile already
--   exists before email confirmation.
-- - Keep the trigger function hardened with an empty fixed search_path and no
