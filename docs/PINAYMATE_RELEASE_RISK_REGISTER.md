...
| R2 | Critical | Secret hygiene | PM_App release-local is expected to fail while a tracked `.env` cleanup/rotation decision remains unresolved. | Engineering owner + release/security approver | Manager summary and signoff checklist identify tracked `.env` as the first release-local blocker. | Approved git-index/history cleanup plan, secret rotation decision, and `npm run check:secret-hygiene` pass output captured without secret values. | `npm run check:release-local` can proceed past secret hygiene, and any exposed/shared key risk is either rotated or formally accepted. |
...
| R6 | High | Dependencies | PM_App has known Expo-chain moderate advisory risk; forced audit fixes can create runtime regression risk. | Engineering owner | Dependency audit gate note calls out Expo-chain advisories and warns against release-branch force fixes. | Fresh dependency audit output, remediation branch evidence or explicit accepted-risk note, then local quality/native QA rerun after any dependency changes. | PM_App dependency audit is clean or accepted by release/security with documented rationale and no untested Expo runtime changes on the release branch. |
| R7 | High | Local release gates | PM_App and PM_Web local quality evidence is stale after later source/script/docs changes. | Codex/engineering QA lane | Manager summary and evidence packet say both local quality domains need rerun after latest patches. | PM_App `npm run check:local-quality`; PM_App `npm run check:release-local` after secret cleanup; PM_Web `npm run check:local-quality`; fresh evidence paths. | Current-head local gates pass, with any warnings triaged and evidence attached before external staging/prod signoff. |
