...
| Check | Result | Evidence note |
| --- | --- | --- |
| Latest functional dev CI | Pass | `PM_App CI` passed for `da013cb` on pull request run `28553689477` and push run `28553687645`. |
| Manual beta preview CI | Pass | GitHub Actions run `28553876248` completed `PM_App CI` and `Beta preview smoke` successfully against `https://beta.pinaymate.com`, covering no-login preview, role switch, seeded messages, seeded chat local send, demo safety report/block, demo Liked You filter/report/block/unmatch/chat with local-only receipts, demo discovery filter save/update, and responsive bottom navigation. |
...
| Local beta preview smoke | Pass | `npm run smoke:beta-preview` passed 8/8 against `https://beta.pinaymate.com` on 2026-07-02. This run covered no-login Foreigner and Pinay previews, role switch, auth entry signup/user-type/recovery validation without backend-changing submits, seeded messages, seeded chat local send, demo safety report/block, demo Liked You filter/report/block/unmatch/chat with local-only receipts, demo discovery filter save/update, and responsive bottom navigation. |
...
| Beta deployment | Pass | Vercel `pm-app-beta` deployment target `https://vercel.com/romega-solutions/pm-app-beta/5rGnb5fZVRTgmMZ11nSzXhv9YhoT` completed for `da013cb` and aliased to `beta.pinaymate.com`. |
