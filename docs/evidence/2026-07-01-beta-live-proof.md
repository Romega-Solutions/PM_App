...
Date: 2026-07-02
Branch/source: `dev` functional source through deployed app commit `d17a2a7`
...
| Check | Result | Evidence note |
| --- | --- | --- |
| Latest functional dev CI | Pass | `PM_App CI` passed for `d17a2a7` on pull request run `28544940109` and push run `28544937638`. |
...
| Manual authenticated web MVP CI | Pass | GitHub Actions run `28545143537` completed `PM_App CI` and `Web MVP live smoke` successfully using repo secrets `PM_WEB_MVP_EMAIL` and `PM_WEB_MVP_PASSWORD`; no credential values were logged. The run covers sign-in, core tabs, setup screens, authenticated profile edit save/restore, protected settings routes, authenticated privacy save/restore, authenticated match preference save, and authenticated notification save/restore. |
| Local authenticated web MVP smoke | Superseded by CI | The strengthened profile-edit smoke requires the deployed `d17a2a7` web bundle. Local checks covered lint, typecheck, unit tests, build, and secret hygiene before deploy; GitHub Actions run `28545143537` is the authoritative browser proof for the updated smoke suite. |
...
| Beta deployment | Pass | Vercel deployment `dpl_HupFKZBZXqMJL6ncWoZVm8GYa8sK` completed for `d17a2a7`, reached `READY`, and aliased to `beta.pinaymate.com`. |
