...
| Local beta preview smoke | Pass | `npm run smoke:beta-preview` passed 8/8 against `https://beta.pinaymate.com` on 2026-07-02. This run covered no-login Foreigner and Pinay previews, role switch, auth entry signup/user-type/recovery validation without backend-changing submits, seeded messages, seeded chat local send, browser picker demo chat photo insertion, demo safety report/block, demo Liked You filter/report/block/unmatch/chat with local-only receipts, demo discovery filter save/update, and responsive bottom navigation. |
